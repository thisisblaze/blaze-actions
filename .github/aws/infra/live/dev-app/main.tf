...
data "terraform_remote_state" "network" {
  ...
  config = {
    ...
    key    = "infra/${var.project_key}/dev/network.tfstate"
    region = var.aws_region
  }
}
...
locals {
  ...
  alb_dns_name = coalesce(try(local.net_outputs.alb_dns_name, ""), try(local.net_config.alb_dns_name, ""), "dev-alb-placeholder.com")
...
module "label" {
  source    = "github.com/thisisblaze/blaze-terraform-infra-core//modules/common/label?ref=v1.50.4"
  client    = var.client_key
  project   = var.project_key
  stage     = var.stage
  namespace = var.namespace
...
# --------------------------------------------------------------------------------
module "app" {
  source = "github.com/thisisblaze/blaze-terraform-infra-core//modules/aws/ecs/environment-app?ref=v1.50.4"
  ...
  enable_frontend_cdn = false # Delegated to dev-network (CloudFront Upfront)
...
module "pages_project_admin" {
  source = "github.com/thisisblaze/blaze-terraform-infra-core//modules/cloudflare/pages-project?ref=v1.50.4"
