# ─────────────────────────────────────────────────────────────
# APPLICATION INFRASTRUCTURE (GCP PROD)
# ─────────────────────────────────────────────────────────────
...
module "environment_app" {
  source = "github.com/thisisblaze/blaze-terraform-infra-core//modules/gcp/compute/environment-app?ref=v2.11.14"
  ...
  api_subdomain_override      = "api-gcp"
  frontend_subdomain_override = "gcp"
...
locals {
  ...
  admin_hostname = local.is_prod ? "admin-gcp.${var.domain_root}" : "admin-gcp-${var.stage}.${var.domain_root}"
}
...
# FIXME: cloudflare_pages_domain is broken in v5 provider (missing required domain_name parameter in API call)
