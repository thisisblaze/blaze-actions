...
variable "api_cpu" {
  ...
  type        = string
  default     = "2" # Higher CPU for prod
}

variable "api_memory" {
  description = "Memory limit for API (e.g. '512Mi', '1Gi')"
  type        = string
  default     = "1Gi" # Higher memory for prod
}

variable "api_min_instances" {
  description = "Minimum instances (0 = scale to zero)"
  type        = number
  default     = 2 # Always-on with redundancy for prod
}
...
variable "use_vpc_connector" {
  ...
  type        = bool
  default     = true # Enabled for prod (private data access)
}
