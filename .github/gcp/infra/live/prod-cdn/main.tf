# ─────────────────────────────────────────────────────────────
# CDN + GLOBAL LOAD BALANCER (GCP PROD)
# ─────────────────────────────────────────────────────────────
# Same as stage-cdn but with:
#   - Tighter rate limits
...
# ── Label ──
module "label" {
  source = "github.com/thisisblaze/blaze-terraform-infra-core//modules/common/label?ref=v2.11.12"
...
# ─────────────────────────────────────────────────────────────
# CLOUD ARMOR (WAF) — Full production protection
# ─────────────────────────────────────────────────────────────

module "cloud_armor" {
  source = "github.com/thisisblaze/blaze-terraform-infra-core//modules/gcp/security/cloud-armor?ref=v2.11.12"
  ...
  rate_limit_requests_per_interval = 60 # Tighter for prod
  rate_limit_interval_seconds      = 60
}

# ─────────────────────────────────────────────────────────────
# IMAGE RESIZE — Warm instances for production
# ─────────────────────────────────────────────────────────────

module "image_resize" {
  source = "github.com/thisisblaze/blaze-terraform-infra-core//modules/gcp/compute/image-resize?ref=v2.11.12"
  ...
  force_destroy      = false # Production — protected

  enable_cdn = false # Global LB handles CDN

  function_min_instances = 1 # Keep warm in prod
  function_max_instances = 20
  function_memory_mb     = 1024 # More memory for larger images
}

# ─────────────────────────────────────────────────────────────
# GLOBAL HTTPS LB + CLOUD CDN — Production optimized
# ─────────────────────────────────────────────────────────────

module "global_lb" {
  source = "github.com/thisisblaze/blaze-terraform-infra-core//modules/gcp/cdn/global-https-lb?ref=v2.11.12"
  ...
  backend_services = [
    ...
    },
    {
      ...
      cdn_default_ttl = 14400   # 4 hours
      cdn_max_ttl     = 2592000 # 30 days
      security_policy = module.cloud_armor.policy_self_link
    },
    {
      name                     = "image-resize"
  ...
  enable_logging  = true
  log_sample_rate = 0.25 # Lower sample rate for prod volume
}
...
resource "cloudflare_dns_record" "lb_dns" {
  ...
  name    = "gcp-prod" # gcp-prod.thisisblaze.uk → GCP Global LB
  type    = "A"
